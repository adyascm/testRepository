...
def oauth_callback(oauth_code, scopes,state, error):
    ...
    if login_user:
        ...
        existing_domain_user = db_session.query(DomainUser).filter(and_(DomainUser.email == login_email, DomainUser.member_type == constants.EntityExposureType.INTERNAL.value)).first()

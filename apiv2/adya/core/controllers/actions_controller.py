...
def add_resource_permission(auth_token, datasource_id, action_payload):
    ...
    new_permissions = messaging.trigger_post_event(urls.ACTION_PATH, auth_token, query_params, {"permissions": json.loads(body)}, "gsuite", constants.TriggerType.SYNC)
...
def update_or_delete_resource_permission(auth_token, datasource_id, action_payload):
    ...
    if action_payload['key'] == action_constants.ActionNames.DELETE_PERMISSION_FOR_USER:
        updated_permissions = messaging.trigger_delete_event(urls.ACTION_PATH, auth_token, query_param, {"permissions": json.loads(body)},
                                                             "gsuite", constants.TriggerType.SYNC)
    else:
        updated_permissions = messaging.trigger_update_event(urls.ACTION_PATH, auth_token, query_param, {"permissions": json.loads(body)},
                                                             "gsuite", constants.TriggerType.SYNC)
...
def execute_batch_delete(auth_token, datasource_id, user_email, initiated_by, permissions_to_update):
    ...
    while sent_perms_count < permissions_to_update_count:
        ...
        if permissions_to_update_count < BATCH_COUNT:
            deleted_response = messaging.trigger_delete_event(urls.ACTION_PATH, auth_token, query_param, {"permissions": json.loads(body)},"gsuite", constants.TriggerType.SYNC)
        else:

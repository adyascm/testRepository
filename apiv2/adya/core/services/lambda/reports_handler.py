...
def get_users_list(event, context):
    req_session = RequestSession(event)
    req_error = req_session.validate_authorized_request(optional_params=["userName", "userEmail", "userType", "sortColumnName", "sortOrder"])
    if req_error:
        return req_error
    users = directory_controller.get_users_list(req_session.get_auth_token(), req_session.get_req_param("userName"), req_session.get_req_param("userEmail"), req_session.get_req_param("userType"), req_session.get_req_param("sortColumnName"), req_session.get_req_param("sortOrder"))

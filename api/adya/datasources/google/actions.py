...
def transfer_ownership(auth_token, domain_id, old_owner_email, new_owner_email):
    try:
...
def transfer_ownership_of_resource(auth_token, domain_id, datasource_id, resource_id, old_owner_email, new_owner_email):
    try:
        print ""
        permission_object = {
        ...
        resource_actions_handler = AddOrUpdatePermisssionForResource(auth_token, domain_id, datasource_id, resource_id,
                                                                     [permission_object], old_owner_email)
...
def remove_external_access_for_all_files_owned_by_user(auth_token, domain_id, datasource_id, user_email):
    try:
        ...
        for resource_id in file_list:
            ...
            resource_actions_handler = AddOrUpdatePermisssionForResource(auth_token, domain_id, datasource_id,
                                                                         resource_id,
...
def remove_external_access_to_resource(auth_token, domain_id, datasource_id, resource_id):
    try:
        print ""
        resource_permissions = resourceController.get_external_permissions_for_resource(domain_id, datasource_id,
                                                                                        resource_id)
...
def make_all_files_owned_by_user_private(auth_token, domain_id, datasource_id, user_email):
    try:
        print ""
...
def make_resource_private(auth_token, domain_id, datasource_id, resource_id):
    try:
        print ""
        resource_permissions = resourceController.get_all_permissions_for_resource(domain_id, datasource_id,
                                                                                   resource_id)
...
def remove_permissions_of_user_to_resource(auth_token, domain_id, datasource_id, resource_id, user_email,
...
def update_permissions_of_user_to_resource(auth_token, domain_id, datasource_id, resource_id, user_email,
                                           new_permission_role, resource_owner_email):
    try:
        print ""
...
def remove_all_access_for_user(auth_token, domain_id, datasource_id, user_email):
    try:
...
class AddOrUpdatePermisssionForResource():
    def __init__(self, auth_token, domain_id, datasource_id, resource_id, permissison_object_batch_data, owner_email):
        self.resource_id = resource_id
        self.batch_data = permissison_object_batch_data
        self.owner_email = owner_email
        db_session = db_connection().get_session()
        datasource = db_session.query(models.DataSource).filter(
            and_(models.DataSource.domain_id == domain_id, models.DataSource.datasource_id == datasource_id)).first()
    ...
    def get_permission_data_object(self, permission_object):
        ...
        if not permission_id:
            ...
            permissiondata = self.delete_permisssion_for_resource(self.drive_service, self.resource_id,
                                                                  permission_object)

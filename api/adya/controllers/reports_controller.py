...
def get_widget_data(auth_token, widget_id):
    ...
    if widget_id == 'usersCount':
        ...
        data = session.query(Resource).filter(Resource.domain_id == LoginUser.domain_id & Resource.resource_type != 'folder').filter(
            LoginUser.auth_token == auth_token).count()
        ...
        data = {}
        data["rows"] = session.query(Resource.resource_name, Resource.resource_type).filter(Resource.domain_id == LoginUser.domain_id & (Resource.exposure_type == constants.ResourceExposureType.EXTERNAL | Resource.exposure_type == constants.ResourceExposureType.PUBLIC)).filter(
            LoginUser.auth_token == auth_token).limit(5).all()
        data["totalCount"] = session.query(Resource.resource_name, Resource.resource_type).filter(Resource.domain_id == LoginUser.domain_id & (Resource.exposure_type == constants.ResourceExposureType.EXTERNAL | Resource.exposure_type == constants.ResourceExposureType.PUBLIC)).filter(
            LoginUser.auth_token == auth_token).count()
    elif widget_id == 'externalUsersList':
        data = {}
        data["rows"] = session.query(DomainUser.email).filter(DomainUser.domain_id == LoginUser.domain_id & DomainUser.member_type == constants.UserMemberType.EXTERNAL).filter(
            LoginUser.auth_token == auth_token).limit(5).all()
        data["totalCount"] = session.query(DomainUser.email).filter(DomainUser.domain_id == LoginUser.domain_id & DomainUser.member_type == constants.UserMemberType.EXTERNAL).filter(
            LoginUser.auth_token == auth_token).count()
    return data

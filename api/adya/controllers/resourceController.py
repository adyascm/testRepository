...
from adya.db.models import Resource,ResourcePermission,LoginUser,DataSource,ResourcePermission,ResourceParent
        ...
        if emailList:
            ...
            if not parent_id:
                ...
                resource_permissions_query_data = db_session.query(Resource,ResourcePermission).outerjoin(ResourcePermission,
                                        ...
                                        .filter(and_(Resource.domain_id == domain_id,
                                        ResourcePermission.email.in_(emailList),ResourceParent.parent_id == parent_id)).all()
            resource_id_set = Set()
            for row in resource_permissions_query_data:
                resource_id_set.add(row.Resource.resource_id)
            resource_data =[]
            for row in resource_permissions_query_data:
                if (row.ResourceParent.parent_id != None and not row.ResourceParent.parent_id in resource_id_set) \
                    or row.ResourceParent.parent_id == None :
            ...
            if not parent_id:
                resources = db_session.query(Resource,ResourcePermission).outerjoin(ResourcePermission,
                                    ...
                                    .outerjoin(ResourceParent,and_(ResourceParent.resource_id == Resource.resource_id,
                                        ResourceParent.domain_id == Resource.domain_id))\
                                    .filter(and_(Resource.domain_id == domain_id, 
                                    Resource.resource_type =='folder',ResourceParent.parent_id == parent_id)).all()
            else:
                resources = db_session.query(Resource,ResourcePermission).outerjoin(ResourcePermission,
                                    and_(ResourcePermission.resource_id == Resource.resource_id,
                                    ResourcePermission.domain_id == Resource.domain_id))\
                                    ...
                                    ResourceParent.domain_id == Resource.domain_id))\
                                    .filter(and_(Resource.domain_id == domain_id, 
                                    ResourceParent.parent_id == parent_id)).all()

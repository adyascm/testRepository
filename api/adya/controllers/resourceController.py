...
def get_resource_tree(auth_token, parent_id,emailList=None):
    ...
    for datasource in datasource_id_list_data:
        ...
        if emailList:
            ...
            resource_data =[]
            for row in resource_permissions_query_data:
                if (row.ResourceParent.resource_parent_id != None and not row.ResourceParent.resource_parent_id in resource_id_set) \
                    or row.ResourceParent.resource_parent_id == None :
            ...
            if not parent_id:
                resources = db_session.query(Resource,ResourcePermission).outerjoin(ResourcePermission,
                                    ...
                                    .outerjoin(ResourceParent,and_(ResourceParent.resource_id == Resource.resource_id,\
                                        ResourceParent.domain_id == Resource.domain_id))\
                                    .filter(and_(Resource.domain_id == domain_id, 
                                    Resource.resource_type =='folder',ResourceParent.resource_parent_id == parent_id)).all()
            else:
                resources = db_session.query(Resource,ResourcePermission).outerjoin(ResourcePermission,
                                    and_(ResourcePermission.resource_id == Resource.resource_id,
                                    ResourcePermission.domain_id == Resource.domain_id))\

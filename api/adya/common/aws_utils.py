...
def create_cloudwatch_event(cloudwatch_event_name, cron_expression):
    try:
        ...
        if lambda_function:
            print "Did not find the report trigger lambda to be attached to cloud watch event, skipping creating the cloud watch event"
            return False
        # Put an event rule
        ...
        print "Exception occurred while creating the cloudwatch event - " + ex
        return False
